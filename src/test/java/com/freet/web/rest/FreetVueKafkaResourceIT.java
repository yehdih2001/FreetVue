...
import com.freet.config.KafkaProperties;
...
class FreetVueKafkaResourceIT {
    ...
    void setup() {
        ...
        FreetVueKafkaResource kafkaResource = new FreetVueKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/freet-vue-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/freet-vue-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
